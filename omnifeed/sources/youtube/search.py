...
class YouTubeSearchProvider(SearchProvider):
    ...
    async def search(self, query: str, limit: int = 10) -> list[SourceSuggestion]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{YOUTUBE_API_BASE}/search", params=params, timeout=30.0)
    ...
    async def _get_subscriber_counts(self, client: httpx.AsyncClient, channel_ids: list[str]) -> dict[str, int]:
        if not channel_ids:
            return {}

        params = {"key": self.api_key, "part": "statistics", "id": ",".join(channel_ids)}
        response = await client.get(f"{YOUTUBE_API_BASE}/channels", params=params, timeout=30.0)
