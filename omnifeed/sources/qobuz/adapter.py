...
QOBUZ_APP_ID = "950096963"  # Default from web player
...
class QobuzAdapter(SourceAdapter):
    ...
    def credentials(self) -> dict:
        if self._credentials is None:
            self._credentials = get_qobuz_credentials()
            if self._app_id:
    ...
    def _api_request(self, endpoint: str, params: dict | None = None) -> dict:
        ...
        request_params = {"app_id": self.credentials["app_id"], **(params or {})}
        ...
        try:
            ...
            if e.response.status_code == 401:
                raise ValueError("Qobuz API authentication failed.")
    ...
    def resolve(self, url: str) -> SourceInfo:
        ...
        artist_data = self._api_request("artist/get", {"artist_id": artist_id, "extra": "albums", "limit": 100})

        name = artist_data.get("name", f"Artist {artist_id}")
        image = artist_data.get("image", {})
        avatar_url = image.get("extralarge") or image.get("large") or image.get("medium") or image.get("small")
    ...
    def poll(self, source: SourceInfo, since: datetime | None = None) -> list[RawItem]:
        ...
        for album in albums:
            ...
            thumbnail = image.get("large") or image.get("medium") or image.get("small") or image.get("thumbnail")

            artist_name = album.get("artist", {}).get("name", source.display_name)
            album_type = album.get("release_type", "album").lower() if album.get("release_type") else "album"
            ...
            raw_item = RawItem(
                ...
                raw_metadata={
                    ...
                    "qobuz_url": album_url,
                    "enclosures": [{"url": album_url, "type": "audio/qobuz"}],
                },
            )
...
class QobuzLabelAdapter(SourceAdapter):
    ...
    def resolve(self, url: str) -> SourceInfo:
        ...
        data = self._api_request("label/get", {"label_id": label_id, "extra": "albums", "limit": 1})
